...
{
    return HAL_I2C_Mem_Read(&hi2c1,DevAddr,MemAddr,1,Data1,DataLen,1000);
}

int8_t Sensor_I2C1_Write(uint16_t DevAddr,uint16_t MemAddr,uint8_t *Data2,uint8_t DataLen)
{
    return HAL_I2C_Mem_Write(&hi2c1,DevAddr,MemAddr,1,Data2,DataLen,1000);
}
...
int8_t MPU6050_Init(int16_t Addr)
{
    ...
    if(check==0x68)//确认设备
    {
        ...
        check=0x07;
        Sensor_I2C1_Write(Addr,SAMPLE_RATE_DRIVER,&check,1);//1khz的速率
        check=0x00;
        Sensor_I2C1_Write(Addr,ACCEL_CONFIG,&check,1);//加速度配置
        check=0x00;
        Sensor_I2C1_Write(Addr,GYRO_CONFIG,&check,1);//陀螺仪配置
        return 0;
    }
    return -1;
}
...
void MPU6050_Read_Accel(void)
{
    ...
    //寄存器依次为 加速度X高-加速度X低-加速度Y高位-加速度Y低位-加速度Z高位-加速度Z低位
...
{
    uint8_t ReadBuf[2];
    Sensor_I2C1_Read(Mpu6050Addr,TEMP_OUT_H,ReadBuf,2);
    MPU6050_Data.Temp=(int16_t)(ReadBuf[0]<<8|ReadBuf[1]);
